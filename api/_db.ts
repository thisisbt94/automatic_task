...
// Add the Neon integration from Vercel's Storage tab and it injects
// DATABASE_URL automatically. See README "Vercel + Postgres deployment".
const connectionString = process.env.DATABASE_URL || process.env.POSTGRES_URL;
if (!connectionString) {
  // Thrown lazily (not at import time in a build step) — real failure
  // happens the first time a request actually needs the database.
  console.warn("[booth] DATABASE_URL / POSTGRES_URL is not set — database calls will fail.");
}
// A syntactically-valid but unreachable placeholder so the module can
// still load (and return a clean error per-request) when the real
// connection string hasn't been configured yet, instead of crashing
// the whole function at cold start.
const sql = neon(connectionString || "postgres://unset:unset@localhost/unset", { fullResults: true });
