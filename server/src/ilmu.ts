...
import { AnalyseTaskInput, IlmuResponse, AnalysisResult } from "./types";
...
export async function analyseTask(input: AnalyseTaskInput): Promise<{ result: IlmuResponse; source: "demo" | "ilmu" | "n8n" }> {
  if (DEMO_MODE) {
    return { result: heuristicAnalyse(input), source: "demo" };
  }
  if (AI_PROVIDER === "n8n") {
    const result = await callN8n(input);
    return { result, source: "n8n" };
  }
  const result = await callIlmu(input);
  return { result, source: "ilmu" };
}

// clarifyTask / categoriseTask are thin, named wrappers over the same
// pipeline for callers/tests that want to be explicit about intent.
export const clarifyTask = analyseTask;
export async function categoriseTask(input: AnalyseTaskInput): Promise<string> {
  const { result } = await analyseTask(input);
  if ("needsClarification" in result && result.needsClarification) return "Uncategorised";
  return (result as AnalysisResult).category;
}

// ── Option B: direct ILMU API (OpenAI-compatible) ────────────────
...
async function callIlmu(input: AnalyseTaskInput): Promise<IlmuResponse> {
  ...
  if (!apiUrl || !apiKey) {
    throw new ServiceUnavailableError("ILMU is not configured (ILMU_API_URL / ILMU_API_KEY missing).");
  }
  ...
  if (!res.ok) {
    throw new ServiceUnavailableError(`ILMU returned HTTP ${res.status}`);
  }
...
// ── Option A: n8n automation webhook ──────────────────────────────
...
function buildUserPrompt(input: AnalyseTaskInput): string {
  const history = input.clarificationHistory
    .map((t, i) => `Clarification ${i + 1} — Q: ${t.question} / A: ${t.answer}`)
    .join("\n");
  return [
...
function parseModelJson(text: string): IlmuResponse {
  ...
  if (parsed.needsClarification) {
    return { needsClarification: true, question: String(parsed.question || "Could you tell me a bit more about that?") };
  }
  return parsed as AnalysisResult;
}

// ── DEMO_MODE heuristic analyser ──────────────────────────────────
// A transparent, rule-based stand-in for ILMU so the booth flow can be
// rehearsed and demoed with zero external dependencies. It is clearly
// labelled "source: demo" everywhere it's stored/returned — never
// presented to attendees as a live AI call. See README "Demo mode".
...
function heuristicAnalyse(input: AnalyseTaskInput): IlmuResponse {
  ...
  // Combine original task with any clarification answers for scoring.
  ...
  const category = CATEGORY_RULES.find((c) => c.keywords.some((k) => combined.includes(k))) ?? CATEGORY_RULES[CATEGORY_RULES.length - 1] ?? { id: "other", label: "Other", keywords: [] };
  ...
  return {
    ...
    followUpQuestions: [
      "Where does the information currently come from?",
      "Where does the finished output need to end up?",
    ],
  };
}
...
function pickClarificationQuestion(text: string): string {
  if (text.includes("inbox") || text.includes("email")) {
    return "What takes the most time — reading emails, finding important ones, replying, or organising them?";
  }
  if (text.includes("messy") || text.length < 20) {
    return "Could you describe one specific example of when this happens, step by step?";
  }
...
function buildWorkflow(automatableSteps: string[]): AnalysisResult["suggestedWorkflow"] {
  const middle = automatableSteps.slice(0, 2).map((s, i) => ({
    step: i + 2,
    label: i === 0 ? "Organise" : "AI Draft",
    description: s,
  }));
  return [
